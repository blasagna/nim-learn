...
discard """
use for mulitline comments
or to quickly remove unparsable code
"""

# var for variable declarations and assignments
var 
    letter: char = 'n'
    lang = "N" & "im"
...
# let to declare and bind once
let
    legs = 400
    arms = 2_000
    aboutPi = 3.15

# consts are computed at compile time
const
    debug = true
...
when compileBadCode:
    legs = legs + 1 # error will never be compiled
    const input = readline(stdin) # const values must be known at compile time

# use discard to bypass compiler complaint of unused expression result
discard 1 < 2 
